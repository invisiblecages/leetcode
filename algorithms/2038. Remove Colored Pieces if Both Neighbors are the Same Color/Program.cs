...
public class Solution 
{
  public bool WinnerOfGame(string colors) 
  {
    List<char> pieces = colors.ToList();
    bool firstTurnWins = false;
    char currentPiece = 'A';

    for (int i = 1; i < pieces.Count - 1; i++)
    {
      if (pieces[i - 1] == currentPiece && 
      pieces[i] == currentPiece && 
      pieces[i + 1] == currentPiece)
      {
        pieces.RemoveAt(i);
        firstTurnWins = !firstTurnWins;
        currentPiece = currentPiece == 'A' ? 'B' : 'A';
        i = 0;
      }
    }

    return firstTurnWins;
  }
}