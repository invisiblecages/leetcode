...
public class Solution 
{
  public long TotalCost(int[] costs, int k, int candidates) 
  {
    ...
    while (k-- > 0)
    {
      while (left <= right && leftPQ.Count < candidates)
        leftPQ.Enqueue(costs[left], costs[left++]);

      while (left <= right && rightPQ.Count < candidates)
        rightPQ.Enqueue(costs[right], costs[right--]);

      bool leftValue = leftPQ.TryPeek(out int leftCost, out int _);
      bool rightValue = rightPQ.TryPeek(out int rightCost, out int _);

      if (!rightValue || (leftValue && leftCost <= rightCost))
